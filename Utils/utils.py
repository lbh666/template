...
class Session():
    ...
    def build_log(self):
        ...
        self.logger = logger
        self.writer = SummaryWriter('/output/logs') if recorder['show_tensorboard'] else None
    ...
    def _load_data(self, mode):
        ...
        form = self.config['Data'].get('dataset_type', None)
        self.config['Data']['root'] = self.config[mode]['root']
        dataset = getattr(importlib.import_module("Dataloader.loader"), form)(self.config['Data'])
        ...
        self.logger.info(f"{mode} Dataset size {len(dataset)} @{self.config['Data']['dataset']} has been loaded")
        return loader
