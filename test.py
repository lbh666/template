...
class Train_Session(utils.Session):
    def _build_model(self):
        ...
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='min', factor=0.2, patience=2)
    ...
    def _epoch(self, data_loader, epoch=None):
        ...
        if mode == 'train':
            ...
            self.logger.info(f'{mode} Acc:{acc.avg}, Epoch:{epoch}, loss:{loss.avg:.3f}, {time.avg:.6f}  seconds/batch')
    ...
    def test(self):
        self.net.load_state_dict(torch.load(r'C:\Users\LBH666\Desktop\Digital-handwriting-recognition\Output\default_data_LeNet_08171644\checkpoint.pkl'))
        self.net.eval()
        with torch.no_grad():
            # test each scene
